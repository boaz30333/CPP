...
double solve(const RealVariable &y)
{
    ...
    double c= y.c();
      if (a==0){
          if(b==0)    __throw_logic_error("try divid by 0");
        return (-c/b);
        return double(-c/b);
    }
    else{
        if(double(std::pow(b,2)-double(double(4*a)*c))<0) __throw_logic_error("no real root");
...
std::complex<double> solve(const ComplexVariable &y)
{
    ...
    std::complex<double> c= y.c();
      if (a.imag()==0&&a.real()==0){
             if(b.imag()==0&&b.real()==0)         __throw_logic_error("try divid by 0");
        return (-c/b);
      }
    else{
