...
{
    if(n==0)        throw std::out_of_range{"Can't divide by zero"};
...
double solve(const RealVariable &y)
{
    ...
    double c= y.c();
      if (a==0){
          if(b==0)    throw std::out_of_range{"No solution"};
        return double(-c/b);
    }
    else{
        if(double(std::pow(b,2)-double(double(4*a)*c))<0) throw std::out_of_range{"No real solution"};
...
std::complex<double> solve(const ComplexVariable &y)
{
    ...
    std::complex<double> c= y.c();
      if (a.imag()==0&&a.real()==0){
             if(b.imag()==0&&b.real()==0)         throw std::out_of_range{"no Solution"};
        return (-c/b);
      }
    else{
